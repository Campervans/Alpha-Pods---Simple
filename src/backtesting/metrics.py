...
this module has performance analysis stuff like
...
def calculate_metrics(returns: pd.Series, risk_free_rate: float = 0.0) -> Dict[str, float]:
    """
    calculate a bunch of performance metrics for a return series.
...
def create_performance_report(index_returns: pd.Series, 
    ...
    """
    create comparison report between index and benchmarks.
    
    Args:
        index_returns: CVaR index returns
        benchmark_returns: dict of benchmark name to returns series
        risk_free_rate: annualized risk-free rate
    ...
    """
    # calc metrics for the CVaR index
    cvar_metrics = calculate_metrics(index_returns, risk_free_rate)
    
    # calc metrics for each benchmark
    ...
    # add relative metrics (vs first benchmark)
    if benchmark_returns:
        ...
        # calc tracking error and info ratio
...
def calculate_rolling_metrics(returns: pd.Series, 
    ...
    Args:
        returns: series of returns
        window: rolling window size (default 252 for 1yr)
...
def save_performance_report(report_df: pd.DataFrame, 
                          ...
                          filepath: str):
    """
    Save performance analysis to Excel.
    
    Args:
        report_df: perf metrics comparison DataFrame
    ...
    with pd.ExcelWriter(filepath, engine='xlsxwriter') as writer:
        ...
        # TODO: this is a bit clunky, maybe find a better way
